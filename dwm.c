...
deck(Monitor *m) {
    ...
    for(i = 0, my = gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
        if(i < m->nmaster) {
            ...
            resize(c, m->wx + gappx, m->wy + my, mw - (2*c->bw) - (r*gappx), h - (2*c->bw), False);
            my += HEIGHT(c) + gappx;
        }
        else
            resize(c, m->wx + mw + gappx/ns + gappx/2, m->wy + gappx, m->ww - mw - (2*c->bw) - gappx*(5-ns)/2 - gappx/2, m->wh - (2*c->bw) - 2*gappx, False);
}
...
drawbar(Monitor *m)
{
    ...
    w = TEXTW(m->ltsymbol);
    snprintf(buf, sizeof(buf), "M|%d", m->nmaster);
    nmw = TEXTW(buf);
    drw_setscheme(drw, scheme[SchemeNorm]);
...
{
    /* Focus the next or previous client of the same type (master/stack), so the
     * master and stack areas are kept isolated (separated). */
    Client *c = NULL, *i;
    int c_num = 0;
    int repeat = 1;

    // Get number of the current client
    int sel_num = 0;
    ...
    while (repeat) {
        ...
        if (c) {
            ...
            // We can't focus a client whose type (stack/master) is
            //   different from the currently focused client.
            if (
                (
...
warp(const Client *c)
{
	...
	if (!c) {
		XWarpPointer(dpy, None, root, 0, 0, 0, 0, selmon->wx + selmon->ww/div, selmon->wy + selmon->wh/div);
		return;
	}
	...
	XWarpPointer(dpy, None, c->win, 0, 0, 0, 0, c->w / div, c->h / div);
}
